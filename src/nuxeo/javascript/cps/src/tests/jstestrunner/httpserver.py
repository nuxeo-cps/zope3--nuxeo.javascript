...
taken from: http://aspn.activestate.com/ASPN/Cookbook/Python/Recipe/259148
"""
...
class RequestHandler(asynchat.async_chat,
    ...
    def collect_incoming_data(self,data):
        """Colparameterslect the data arriving on the connexion"""
...
class Server(asyncore.dispatcher):
    ...
    def handle_accept (self):
        try:
            ...
            return
        # creates an instance of the handler class to jsRequestHandlerhandle the request/response
        # on the incoming connexion
        self.handler(conn, addr, self)

class jsRequestHandler(RequestHandler):

    def handle_data(self, *args, **kw):
        if not self.path.startswith('/jsResults?'):
            return None
        results = self.path.split('?')[1]
        results = results[len('result='):]
        self.server.lastResult(results)

from threading import Thread

class JsThreadServer(Thread):
    ...
    def run(self):
        ...
        self.server = Server('', self.port, jsRequestHandler)
        try:
            asyncore.loop(timeout=2)
        except (KeyboardInterrupt, select.error):
            # Crtl+C pressed. or process killed Shutting down.
            pass
